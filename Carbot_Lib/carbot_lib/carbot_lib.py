...
class Carbot(object):
    ...
    # 设置总线舵机角度接口：s_id:[1,6], s_angle: 1-4:[0, 300], 5:[0, 270], 6:[0, 180], 设置舵机要运动到的角度。
    # run_time表示运行的时间(ms),时间越短,舵机转动越快。最小为0，最大为2000
    # Set bus steering gear Angle interface: s_id:[1,6], s_angle: 1-4:[0, 300], 5:[0, 270], 6:[0, 180], set steering gear to move to the Angle.  
    # run_time indicates the running time (ms). The shorter the time, the faster the steering gear rotates.  The minimum value is 0 and the maximum value is 2000
    ...
    def get_car_type_from_machine(self):
        ...
        for i in range(0, 20):
            if self.__read_car_type != -1:
                car_type = self.__read_car_type
                self.__read_car_type = -1
                return car_type
            time.sleep(.001)
        return -1
